CHEAT-SHEET — Disk Management, HTTP Server & DNS Record Types
─────────────────────────────────────────────────────
Core Concepts:
  Disk partitioning and mounting for persistent storage
  Apache2 HTTP server with name-based virtual hosts
  DNS record types for various services (CNAME, MX, TXT)
...
Problem                    │ Solution
───────────────────────────┼─────────────────────────────────────
Device busy (umount fails) │ lsof | grep /m300data (find processes)
                           │ fuser -m /m300data (kill processes)
Mount fails at boot        │ Check /etc/fstab syntax
                           │ Verify LABEL or UUID is correct
...
Problem                        │ Solution
───────────────────────────────┼─────────────────────────────────────
Port 80 already in use         │ ss -tlpn | grep :80
                               │ Stop conflicting service (e.g., Traefik)
Apache won't start             │ apache2ctl -t (check syntax)
                               │ systemctl status apache2 (view errors)
403 Forbidden                  │ Check file permissions
                               │ chown -R www-data:www-data /var/www/site1
404 Not Found                  │ Check DocumentRoot path
                               │ Verify index.html exists
Site shows default page        │ Check ServerName matches DNS
                               │ Reload Apache after config changes
Changes not taking effect      │ systemctl reload apache2
                               │ Clear browser cache
...
## DNS RECORD TYPES (Comprehensive)
...
Result:
www.lan.m300.smartlearn.ch → resolves to vmls3's IP
