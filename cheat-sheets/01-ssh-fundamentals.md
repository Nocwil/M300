CHEAT-SHEET — SSH Fundamentals
─────────────────────────────────────────────────────
Core Concept:
  SSH (Secure Shell) provides encrypted remote access to servers.
  It uses public/private key pairs for authentication instead of 
  passwords, enabling secure automated access and tunneling.
...
Example: Access MySQL on vmLS3 (bound to 127.0.0.1) from vmLP1:
  ssh -L 127.0.0.1:13306:127.0.0.1:3306 vmadmin@192.168.220.13
  Then: mysql -h 127.0.0.1 -P 13306 -u root -p
...
• Mixing up port numbers vs bind addresses (port is "which service",
  bind address is "which network interface")
