CHEAT-SHEET — TLS Certificates & Let's Encrypt
─────────────────────────────────────────────────────
Core Concept:
...
Asymmetric          │ Public key encrypts,            │ Slower
                    │ Private key decrypts (or vice)  │
...
ssh-keygen creates a key pair: id_ed25519 (private) and id_ed25519.pub (public)
...
Level                │ Role                           │ Trust
─────────────────────┼────────────────────────────────┼────────────────
Root CA              │ Top-level authority            │ Pre-installed
                     │ (e.g., DigiCert, IdenTrust)    │ in browsers
                     │                                │
Intermediate CA      │ Signs server certificates      │ Signed by
                     │ (e.g., Let's Encrypt R3)       │ Root CA
                     │                                │
Server Certificate   │ Your website's certificate     │ Signed by
                     │ (e.g., *.m300.smartlearn.ch)   │ Intermediate
...
Why it exists:
After Edward Snowden's 2013 NSA revelations, HTTPS became critical.
Let's Encrypt made certificates free and automatic.
...
──────────────────┼─────────────────────────────────┼──────────────────
HTTP-01           │ Place file at:                  │ Port 80 accessible
                  │ http://domain/.well-known/      │ from internet
                  │ acme-challenge/<token>          │
                  │                                 │
DNS-01            │ Create TXT record:              │ API access to
                  │ _acme-challenge.domain.com      │ DNS provider
...
──────────────────┼─────────────────────────────────┼──────────────────
DNS Server        │ Resolves domain names           │ moduldns.smartlearn.ch
                  │                                 │ vmLS3 (lab DNS)
                  │                                 │
Tunnel Gateway    │ Public entry point              │ modulgw.smartlearn.ch
                  │ Receives SSH tunnels            │
                  │                                 │
Traefik (Gateway) │ Routes traffic to services      │ On modulgw
...
──────────────────────────┼───────────────────────────────┼─────────────────
NET::ERR_CERT_AUTHORITY   │ Self-signed or untrusted CA   │ Accept risk or
_INVALID                  │                               │ get valid cert
                          │                               │
NET::ERR_CERT_COMMON_NAME │ Domain mismatch               │ Certificate CN
_INVALID                  │                               │ must match domain
                          │                               │
Certificate expired       │ Past validity date            │ Renew certificate
                          │                               │
No certificate            │ Traefik can't find cert files │ Check paths,
                          │                               │ permissions
